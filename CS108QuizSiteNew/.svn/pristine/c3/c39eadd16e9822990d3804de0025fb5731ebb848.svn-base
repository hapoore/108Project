...
<body>
<%
Set<String> results = (Set<String>)session.getAttribute("Search Results");
Account account = (Account) session.getAttribute("Account");
String loggedInUsername = account.getUserName();
%>
			...
			<div class="col-xs-12">
				<div class="well">
					...
					if(results == null || results.size() == 0) {
						out.println("<p><h3>Uh oh, looks like your search hasn't turned up any results!</h3></p>");
					} else {
						for(String username: results) {
							String url = null;
							if(username.equals(loggedInUsername)) {
								url = "welcome-user.jsp";
							} else {
								url = "display-user-page.jsp?id=" + username; //Build URL
							}
							out.println("<p><li><a href=" + url + ">" + username + "</a></li></p>");
						}
					}
