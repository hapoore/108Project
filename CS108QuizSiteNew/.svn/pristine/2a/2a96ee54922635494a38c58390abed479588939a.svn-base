	...
	<div class="container">
		<div class="row">
			...
			<div class="col-xs-10" style="top: 70px;">
				...
				<br>
				<%
					LinkedList<Conversation> newM = account.getNewMessages();
					
					if (newM.size() == 0) {
						out.println("You have no new messages at this time.");
					}
					else {
						int size = newM.size();
						for (int i = 0; i < size; i++) {
							Conversation cur = newM.get(i);
							out.println("<div class=\"well\">");
							
							
							Account current = Account.getOnlineAccount(cur.getUserName());
							String imag = "nope";
							if (current == null) {
								imag = "yes";
							} else { //online
							...
							if (cur.getNewNotifications() == 1) {
								out.println("<div>" + cur.getNewNotifications() + "new message");
							} else {
								out.println("<div>" + cur.getNewNotifications() + "new messages");
							}
							out.println("</div>");
							...
							out.println("<input class=\"btn btn-success\" type=\"submit\" name=\"decision\" value=\"Reply\">");
							out.println("<input class=\"btn btn-danger\" type=\"submit\" name=\"decision\" value=\"Ignore\">");
							out.println("</form>");
			...
			<div class="col-xs-2 list-group" style="top: 70px;">
				...
				<a href="inbox.jsp" class="list-group-item">Friend
					Requests</a> <a href="inbox_challenges.jsp" class="list-group-item">Challenges</a>
