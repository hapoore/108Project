	...
	<div class="container">
		<div class="row">
			<div class="col-xs-12">
				<div class="well">
					<%
Account currUser = (Account)session.getAttribute("Account");
out.println("<h1>My History:</h1>");
out.println("<h2>Quizzes Created:</h2>");
LinkedList<Quiz> quizzesMade = currUser.getQuizzesCreated();
if(quizzesMade == null || quizzesMade.size() == 0) {
	out.println("<p>You have not made any quizzes!</p>");
} else {
	for(Quiz quiz: quizzesMade) {
		out.println("<p> " + quiz.toString() + "</p>");
	}
}
out.println("<h2>Quizzes Taken:</h2>");
LinkedList<QuizResult> pastResults = currUser.getPastPerformances();
if(pastResults == null || pastResults.size() == 0) {
	out.println("<p>You have not taken any quizzes!</p>");
} else {
	for(QuizResult result: pastResults) {
